...
CFcsGLControlPanelCollection::CFcsGLControlPanelCollection()
{
    m_iCurrentPanel = -1;
}
...
CFcsGLControlPanelCollection::~CFcsGLControlPanelCollection()
{
    ...
    for(i=0; i<m_vFcsGLControlPanels.size(); i++)
        if(m_vFcsGLControlPanels[i] != NULL)
            delete m_vFcsGLControlPanels[i];
}

CFcsGLControlPanel* CFcsGLControlPanelCollection::AddFcsGLControlPanel(char *pName, int iPosH, int iPosV, int iSizeH, int iSizeV)
{
    CFcsGLControlPanel* pCp = NULL;

    pCp = new CFcsGLControlPanel(pName, iPosH, iPosV, iSizeH, iSizeV);

    if(pCp != NULL)
        m_vFcsGLControlPanels.push_back(pCp);

    return pCp;
}

bool CFcsGLControlPanelCollection::Mouse(int Button, int State, int x, int y)
{
    bool bRet;
    unsigned int i;
    ...
    for(i=0; i<m_vFcsGLControlPanels.size(); i++)
    {
        bRet = m_vFcsGLControlPanels[i]->Mouse(Button, State, x, y);
        if(bRet)
        {
            m_iCurrentPanel = i;
            break;
...
bool CFcsGLControlPanelCollection::Motion(int iX, int iY)
{
    bool bRet;
    unsigned int i;
    ...
    for(i=0; i<m_vFcsGLControlPanels.size(); i++)
    {
        bRet = m_vFcsGLControlPanels[i]->Motion(iX, iY);
        if(bRet)
        {
            m_iCurrentPanel = i;
            break;
...
bool CFcsGLControlPanelCollection::PassiveMotion(int iX, int iY)
{
    bool bRet;
    unsigned int i;
    ...
    for(i=0; i<m_vFcsGLControlPanels.size(); i++)
    {
        bRet = m_vFcsGLControlPanels[i]->PassiveMotion(iX, iY);
        if(bRet)
        {
            m_iCurrentPanel = i;
            break;
...
void CFcsGLControlPanelCollection::Display(void)
{
    unsigned int i;

    for(i=0; i<m_vFcsGLControlPanels.size(); i++)
        if(m_vFcsGLControlPanels[i] != NULL)
            m_vFcsGLControlPanels[i]->Display();
}

CFcsGLControlPanel* CFcsGLControlPanelCollection::GetCurrentPanel()
{
    if( (m_iCurrentPanel < 0) || (m_iCurrentPanel > (m_vFcsGLControlPanels.size() - 1)) )
        return NULL;
    else
        return m_vFcsGLControlPanels[m_iCurrentPanel];
}


CFcsGLControlPanel::CFcsGLControlPanel(char *pName, int iPosH, int iPosV, int iSizeH, int iSizeV)
{
	m_iLeft = iPosH;
...
CFcsGLControlPanel::~CFcsGLControlPanel()
{
	int i;
...
CCheckBox* CFcsGLControlPanel::AddCheckBox(char *pName, bool bVal, bool bEnabled, void (*pCallBack)(bool bNewVal))
{
	int iPosH, iPosV;
	CCheckBox *pC; 
...
CSlider* CFcsGLControlPanel::AddSlider(char *pName, double dVal, double dMinVal, double dMaxVal, bool bEnabled, void (*pCallBack)(double bNewVal))
{
...
COptionBox* CFcsGLControlPanel::AddOptionBox(char *pName, int iVal, bool bEnabled, void (*pCallBack)(int bNewVal))
{
	int iPosH, iPosV, iWidth;
...
CSlider* CFcsGLControlPanel::AddInfoSlider(char *pName, double dVal, double dMinVal, double dMaxVal, bool bEnabled, void (*pCallBack)(double bNewVal))
{
	int iPosH, iPosV, iWidth;
...
CGraphBox* CFcsGLControlPanel::AddGraphBox(char *pName, bool bEnabled, void (*pCallBack)(int iNewVal))
{
	int iPosH, iPosV, iWidth;
...
void CFcsGLControlPanel::SetVisible(bool bVisible)
{
	m_bVisible = bVisible;
}

void CFcsGLControlPanel::Resize(int iPosH, int iPosV, int iSizeH, int iSizeV)
{
	int i, iH, iV, iW;
...
bool CFcsGLControlPanel::Mouse(int iButton, int iState, int iX, int iY)
{
	int i;
	bool bHit=false;
...
bool CFcsGLControlPanel::Motion(int iX, int iY)
{
	int i;
	bool bHit=false;
...
bool CFcsGLControlPanel::PassiveMotion(int iX, int iY)
{
	bool bHit = false;
	int i;
...
void CFcsGLControlPanel::Display(void)
{
	...
	if(m_bVisible)
	{
		...
		//Draw FcsGLControlPanel Title Bar
        ...
        //Draw FcsGLControlPanel Background
...
void CSlider::Display(void)
{
	...
	glColor4dv(CTRL_VAL_COLOR);
	m_iValRight = m_iValLeft + int(m_iPerc / m_dPercPerPix);
	glBegin(GL_POLYGON);
...
void COptionBox::SetValue(int iNewVal)
{
    if(m_bEnabled)
    {
	    if( (iNewVal >= 0) && (iNewVal < m_vOptions.size()) )
	    {
			...
			/*
		    if(mpf_CallBack != NULL)if(mpf_CallBack != NULL)
		    {
			    mpf_CallBack(m_iValue);
		    }
			*/
